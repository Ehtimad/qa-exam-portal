...
export default async function AdminAnalyticsPage() {
  ...
  // All exam attempts for analysis
  ...
  const qMap = new Map(allQuestions.map((q) => [q.id, { ...q, options: JSON.parse(q.options) as string[], correctAnswers: JSON.parse(q.correctAnswers) as number[] }]));
  ...
  for (const attempt of attempts) {
    ...
    const questionOrder: number[] = attempt.questionOrder ? JSON.parse(attempt.questionOrder) : Object.keys(answers).map(Number);
  ...
  const qStatsArr: QStat[] = Array.from(qStats.entries())
    .map(([id, s]) => ({
  ...
  const lecStats: LecStat[] = Array.from(lecMap.entries())
    .map(([lectureId, s]) => ({
  ...
  const top10Hardest = qStatsArr.filter((q) => q.attempts >= 3).slice(0, 10);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    ...
                    <th className="text-right px-3 py-3 text-gray-500 font-medium">Xəta nisbəti</th>
        ...
        <section>
          <h2 className="text-xl font-bold text-gray-900 mb-4">Ən Çətin 10 Sual</h2>
          <div className="card overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                ...
                <tbody>
                  ...
                  {top10Hardest.length === 0 && (
                    <tr><td colSpan={5} className="px-3 py-8 text-center text-gray-400">Məlumat yoxdur (ən az 3 cəhd lazımdır)</td></tr>
                  )}
                </tbody>
              </table>
            </div>
